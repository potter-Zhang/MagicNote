...
+ 慈恩(susong):后端编写。主要负责后端的部分框架的搭建；主要负责后端中部分controller，service以及security，exception的编写；通过docker部署minio实现文件存储与操作；部署OCR模型tessert实现图片文字识别；部署Whipser并利用jsch库连接云服务器实现音视屏文字转换；实现AI功能中生成表格与生成流程图功能的调用与调试。
